<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page
	import="java.util.ArrayList, my_recipe_board.model.vo.MyRecipe, recipe_matelial_list.model.vo.Material, recipe_matelial_list.model.vo.MaterialList"%>
<%
	MyRecipe boardno = (MyRecipe) session.getAttribute("boardno");
...
<%@ include file="../common/header.jsp" %>
	<div id = "main">
		<button onclick="javascript:location.href='/jmtgr/mingrelist'">재료
			전체 보기</button>
		...
		<form action="/jmtgr/mingresearch?mrno=<%=boardno.getMrBoardNo()%>" >
		...
		<center>
			...
			<form method="post"
				id = "listaddform"
				action="/jmtgr/mingreinsert?mrno=<%=boardno.getMrBoardNo()%>"
				>
				<table id="outer" align="center" width="600" cellspacing="5"
					...
					<tr>
						...
						<td><input type="text" name="r_Board_No"
							value="<%=boardno.getMrBoardNo()%>" required></td>
					</tr>
					<tr>
			...
			<table align="center" border="1" cellspacing="0" cellpadding="3"
				...
				<tr>
					<td><%=m.getMaName()%></td>
					<td><%=m.getGram()%></td>
					<td><button onclick="javascript:location.href='/jmtgr/mringredelete?mano=<%= m.getMaNo() %>&mrno=<%= boardno.getMrBoardNo()%>'">삭제 </button></td>
					<%
						}
