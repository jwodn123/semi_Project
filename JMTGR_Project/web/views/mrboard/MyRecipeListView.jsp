<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="my_recipe_board.model.vo.*, java.util.ArrayList"%>
<%
...
function clearText(field){ 
	...
	<div class = "listmain">
		...
		<div class="<%= name[i] %>" style="background-image:url(resources/mrthumbnail/<%= mr.getMrboardrenameFile()%>); background-size:cover;">
		
		<h1 class="title">
			<%= mr.getMrBoardTitle() %>
			<br>
			<% if (loginMember != null && loginMember.getUserId().equals("admin")) {%>
			<button class="view" onclick="javascript:location.href='/jmtgr/mrdetail.ss?mrno=<%= mr.getMrBoardNo() %>'">view</button>
			<button class="add" onclick="javascript:location.href='/jmtgr/mrbno?mrno=<%=mr.getMrBoardNo() %>'">재료추가</button>
	...
	<a href="/jmtgr/orderbyS?orderby=titledesc"> [ 제목 역순 정렬 ]</a>
	</div>
	...
	<!-- 페이징 처리 -->
		...
		<% if ((currentPage - 1)  >= startPage){ %>
			<a class="arrow prev" href="/jmtgr/mrlist.ss?page=<%= startPage - 1 %>"></a>
		<% } else { %>
