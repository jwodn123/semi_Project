...
<body>
	...
	<div class="listtitle"><%= keyword %></div>
	<div id="toolbox">
    ...
    <fieldset class="fld_inside opt_on" style="border:0 solid black;"><!--  내부검색 fld_inside 추가 --><!--  내부검색 옵션이 있을경우 opt_on 추가 -->
        <div class="box_searchbar"><!-- 입력창 선택시 search_on 추가  -->
            ...
            <!--// 옵션창  -->
            <input type="text" class="tf_keyword" id="qTop" name="keyword" value="" title="검색어 입력" autocomplete="off" spellcheck="false" />
	...
	<div id="orderby" >
	<select id="orderbyValue" name="orderbyKeyword" onchange="movePage(this.value)">
	...
	<div class="listmain">
		...
		<div class="<%= name[i] %>" style="background-image:url(resources/thumbnail/<%= kb.getRenameFile() %>); background-size : cover;">
			...
			<h1 class="title">
				...
				<button class="view" onclick="javascript:location.href='/jmtgr/krdetail.ss?krno=<%= kb.getKrBoardNo() %>&page=<%= currentPage %>&keyword=<%= keyword %>&searchKeyword=<%= searchKeyword %>'">	VIEW</button>
	...
	<div class="page_nation">
		<% if(currentPage <= 1) { %>
			...
			&nbsp;
		<% }else{ %>
			<a class="arrow pprev" href="/jmtgr/krsearchT.en?page=1&keyword=<%= keyword %>&searchKeyword=<%= searchKeyword %>"></a>
		<% } %>
		<!-- 이전 그룹으로 이동 처리 -->
		<% if((currentPage - 10) < startPage && startPage != 1){ %>
			<a class="arrow prev" href="/jmtgr/krsearchT.en?page=<%= currentPage = startPage-10 %>&keyword=<%= keyword %>&searchKeyword=<%= searchKeyword %>"></a>
		<% }else { %>
		...
		<% for(int p = startPage; p <= endPage; p++){ 
			...
			<% }else{ %>
			<a href="/jmtgr/krsearchT.en?page=<%= p %>&keyword=<%= keyword %>&searchKeyword=<%= searchKeyword %>"><%= p %></a>
		<% }} %>
		<!-- 다음 그룹으로 이동 처리 -->
		<% if(endPage != maxPage ){ %>
			<a class="arrow next" href="/jmtgr/krsearchT.en?page=<%= currentPage = startPage + 10 %>&keyword=<%= keyword %>&searchKeyword=<%= searchKeyword %>"></a>
		<% }else{ %>
		...
		<% if(currentPage >= maxPage){ %>
			...
			&nbsp;
		<% }else{ %>
			<a class="arrow nnext" href="/jmtgr/krsearchT.en?page=<%= maxPage %>&keyword=<%= keyword %>&searchKeyword=<%= searchKeyword %>"></a>
		<% } %>	
	</div>	
</div>
<hr>
